...
def setup_page():
    """Apply custom css and setup page(withput setting page config)"""
    apply_custom_css()
...
def apply_custom_css(accent_color="#d32f2f"):
    ...
    <style>
        ...
        /* Active tabs anf highlight based on accent color */
        ...
        /* Horizontal layout for strenghts and improvements */
...
def setup_sidebar():
    with st.sidebar:
        ...
        groq_api_key = st.text_input("Groq API Key", type="password", help="Get your free key at console.groq.com")
...
def role_selection_section(role_requirements):
    ...
    custom_jd = None
    if upload_jd:
        custom_jd_file = st.file_uploader("Upload job description (PDF or Text)", type=["pdf", "txt"])
        if custom_jd_file:
        ...
        for weakness in detailed_weaknesses:
            ...
            with st.expander(f"{skill_name} (Score: {score}/10)"):
                ...
                if 'example' in weakness and weakness['example']:
                    st.markdown(
                        f"<strong>Example addition:</strong> {weakness['example']}",
                        unsafe_allow_html=True
                    )
...
"""
        ...
        href = f'<a class="download-btn" href="data:text/plain;base64,{report_b64}" download="agent_resume_analysis.txt">📊 Download Analysis Report</a>'
...
def resume_qa_section(has_resume, ask_question_func=None):
    ...
    if user_question and ask_question_func:
        ...
        st.markdown(
            '<div style="background-color: #111122; padding: 15px; border-radius: 5px; border-left: 5px solid #32f2f;">',
            unsafe_allow_html=True
        )
        st.write(response)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("Generate Interview Questions"):
        if generate_questions_func:
            ...
            for i, (q_type, question) in enumerate(questions):
                ...
                if q_type == "Coding":
                    download_content += """```python # Write your solution here ```\n\n"""
...
def resume_improvement_section(has_resume, improve_resume_func=None):
    ...
    if st.button("Generate Resume Improvements"):
        if improve_resume_func:
            ...
            download_content += f"Target Role: {target_role if target_role else 'Not specified'}\n\n"
            ...
            for area, suggestions in improvements.items():
                with st.expander(f"Improvements for {area}", expanded=True):
                    ...
                    download_content += f"### Specific Suggestions\n\n"
            ...
            href = f'<a class="download-btn" href="data:text/markdown;base64,{b64}" download="Agent_resume_improvement.md">📝 Download All Suggestions</a>'
            st.markdown(href, unsafe_allow_html=True)
